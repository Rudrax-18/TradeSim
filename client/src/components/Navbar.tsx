...
import { TrendingUp, Wallet, LayoutDashboard, Briefcase, RefreshCw, Star, LogOut, Sun, Moon, ClipboardList, History } from 'lucide-react';
...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 theme-bg-nav border-b px-6 py-3.5 shadow-sm transition-all duration-300">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        {/* Brand Logo */}
        <div className="flex items-center gap-2 cursor-pointer shrink-0" onClick={() => navigate('/dashboard')}>
          <div className="bg-gradient-to-tr from-emerald-500 to-teal-400 p-2 rounded-xl text-[#0b0f19]">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto whitespace-nowrap scrollbar-none max-w-full pb-1 md:pb-0 -mx-4 px-4 md:mx-0 md:px-0">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.to}
              className={({ isActive }) =>
                `flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-300 border ${
                  isActive
        ...
        {/* User Status Section */}
        <div className="flex items-center flex-wrap md:flex-nowrap justify-between md:justify-end gap-4 border-t border-[var(--color-border-card)] md:border-none pt-3.5 md:pt-0">
